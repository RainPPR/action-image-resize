...
它能将 PNG、JPG 等位图全自动转换为现代的 **AVIF** 格式，对 SVG 应用 [SVGO](https://github.com/svg/svgo) 优化，并在转换后**自动同步所有 Markdown 中的图片引用路径**，让你无需任何手动操作。
...
- **SVG 优化处理**
  所有 SVG 文件通过 [SVGO](https://github.com/svg/svgo) 进行 `multipass` 极限优化，包括属性排序、ID 去重、样式转换等优化项。

- **超宽图片限制**  
  位图宽度超过 `2560px` 时，自动等比缩放至 `2560px`。
...
- **SVG 数值精度**：SVGO 优化会将 SVG 中的浮点数精度适当降低，对常规屏幕显示无感，但不适合作为高精度矢量编辑的源文件存储。
- **重复运行**：每次运行所有 SVG 都会被 SVGO 再次处理，请注意控制触发频率。
