...
- ❌ **摄影或高保真作品集**：本项目默认采用 `avif` 质量 `65` 且会强制对超大图进行缩放，不适合对图像细节有严苛要求的场景。
...
- **矢量图(SVG)智能策略**：
  - **>= 100KB**：强制转换为 `avif` 格式。
  - **40KB - 100KB**：若包含内联位图或字体，强制转为 `avif`；否则根据压缩比决定。
  - **10KB - 100KB**：尝试转为 `avif`，若压缩效果显著（>40KB 需减小 50%，<40KB 需减小 80%）则转换。
  - **其他/未转换**：基于 [SVGO](https://github.com/svg/svgo) 进行极限优化，保持矢量格式。
- **智能缩放策略**：图片宽度上限限制在 `1080px`，超宽图片将自动等比缩小。
...
2. **Sharp 转码**：
   ...
   - 转换为 `avif` 格式（质量 65），删除原图。
3. **SVG 处理**：
   - **强制转码**：体积 >= 100KB，或体积 >= 40KB 且含有内联位图/字体。
   - **动态转码**：10KB - 100KB 之间的文件，若转换为 `avif` 后体积大幅减小（>40KB 需 <0.5倍，<40KB 需 <0.2倍）则执行转换。
   - **SVGO 优化**：所有保留为 `.svg` 的文件均执行 SVGO `multipass` 优化。
